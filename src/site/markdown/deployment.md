...
* Click any of the [YADA-Quickstart] links throughout this document to download the `YADA-Quickstart-${releaseVersion}.war` file, or
...
After making your desired changes, simply run `mvn package` in the `YADA-Quickstart` directory.  This will result in a `YADA-Quickstart-${releaseVersion}.war` file in the `target` subdirectory.  This warfile can now be copied to your tomcat container.
...
cp ~/YADA-Quickstart-${releaseVersion}/src/main/resources/dev/yada.httpd.conf .
